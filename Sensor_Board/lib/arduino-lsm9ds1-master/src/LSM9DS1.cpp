...
int LSM9DS1Class::begin()
{
  ...
  writeRegister(LSM9DS1_ADDRESS, LSM9DS1_CTRL_REG1_G, 0xBB); // 476 Hz, 2000 dps, 100 Hz BW
  writeRegister(LSM9DS1_ADDRESS, LSM9DS1_CTRL_REG6_XL, 0xA0); // 476 Hz, 2G
...
void LSM9DS1Class::setAccelerationRange(uint8_t range){
  uint8_t reg = readRegister(LSM9DS1_ADDRESS, 0x20);

  reg &= ~(0b00011000);
  reg |= range;

  writeRegister(LSM9DS1_ADDRESS, 0x20, reg);
}
